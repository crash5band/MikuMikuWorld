...
namespace IO
{
    ...
    public:
        ...
        void update(const std::vector<uint8_t>& b);
        std::string final();
    ...
    inline void SHA1::update(const std::vector<uint8_t>& b)
    {
        size_t totalRead = 0;
        while (true)
        {
            auto read = std::min(BLOCK_BYTES - buffer.size(), b.size() - totalRead);
            std::copy_n(b.begin() + totalRead, read, std::back_inserter(buffer));
