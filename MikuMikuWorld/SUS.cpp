...
namespace MikuMikuWorld
{
	...
	void saveSUS(const SUS& score, const std::string& filename)
	{
		...
		{
			int startTick = slide[0].tick;
			int endTick = slide[slide.size() - 1].tick;
			int channel = provider.generateChannel(startTick, endTick);
			...
			std::string line = "3";
			for (const auto& step : slide)
			{
				char buff2[10];
				std::string info = line + tostringBaseN(buff2, step.lane, 36) + chStr;

				for (const auto& [barLength, barTicks] : barLengthTicks)
				{
					if (step.tick >= barTicks)
					{
						int currentMeasure = barLength.bar + ((step.tick - barTicks) / ticksPerBeat / barLength.length);
						info = "#" + formatString("%03d", currentMeasure) + info + ":";
						int notePos = step.tick - barTicks - barLength.length * ticksPerBeat * (currentMeasure - barLength.bar);
						int gcd = barLength.length * ticksPerBeat;
						gcd = std::gcd(notePos, gcd);
						int lineLength = barLength.length * ticksPerBeat / gcd;
						notePos = notePos / gcd;
						for (int i = 0; i < lineLength; i++)
						{
							info.append(i == notePos ? (std::to_string(step.type) + tostringBaseN(buff2, step.width, 36)) : "00");
						}
		...
		for (const auto& [tag, map] : noteMaps)
		{
			...
			for (const auto& raw : map.data)
			{
				bool newFlag = true;
				int lineIndex = 0;
				int dataIndex = raw.tick % map.ticksPerMeasure;
				for (int i = 0; i < data.size(); i++)
				{
					if (data[i].find(dataIndex) == data[i].end())
					{
						lineIndex = i;
						newFlag = false;
						break;
				...
				data[lineIndex][dataIndex] = raw.data;
			}
			...
			{
				values.push_back("");
				for (int j = 0; j < map.ticksPerMeasure; j += gcd)
				{
					if (data[i].find(j) != data[i].end())
						values[i].append(data[i][j]);
					else
						values[i].append("00");
				}
				lines.push_back(line + values[i]);
			}
			//
