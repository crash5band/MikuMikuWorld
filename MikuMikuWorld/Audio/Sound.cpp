...
namespace Audio
{
	...
	void Sound::initialize(std::string name, ma_uint32 sampleRate, ma_uint32 channelCount, ma_uint64 frameCount,
	                       int16_t* samples)
	{
		this->name = name;
		...
		ma_audio_buffer_config bufferConfig =
		    ma_audio_buffer_config_init(this->sampleFormat, channelCount, frameCount, this->samples.get(), nullptr);
	...
	mmw::Result decodeAudioFile(std::string filename, Sound& sound)
	{
		...
		if (fileExtension == ".mp3")
		{
			...
			uint64_t frameCount{};
			int16_t* samples =
			    drmp3_open_memory_and_read_pcm_frames_s16(bytes.data(), bytes.size(), &mp3Config, &frameCount, nullptr);
			if (samples == nullptr)
		...
		else if (fileExtension == ".wav")
		{
			...
			uint32_t sampleRate{};
			uint64_t frameCount{};
			int16_t* samples = drwav_open_memory_and_read_pcm_frames_s16(bytes.data(), bytes.size(), &channels,
		...
		else if (fileExtension == ".flac")
		{
			...
			uint32_t sampleRate{};
			uint64_t frameCount{};
			int16_t* samples = drflac_open_memory_and_read_pcm_frames_s16(bytes.data(), bytes.size(), &channels,
			                                                              &sampleRate, &frameCount, nullptr);
			if (samples == nullptr)
	...
	void SoundPool::initialize(const std::string& path, ma_engine* engine, ma_sound_group* group, SoundFlags flags)
	{
		...
		{
			ma_result result =
			    ma_sound_init_from_file_w(engine, wPath.c_str(), maSoundFlags, group, nullptr, &pool[i].source);
	...
	void SoundPool::play(float start, float end)
	{
		if (flags & SoundFlags::EXTENDABLE)
		{
			...
			// If playback is started on a note, the source's time is effectively 0 and the sound
			// isn't marked playing yet
			const bool isCurrentInstancePlaying =
			    ma_sound_is_playing(&currentInstance.source) ||
