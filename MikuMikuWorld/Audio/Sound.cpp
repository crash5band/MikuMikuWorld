...
namespace Audio
{
	...
	void Sound::initialize(std::string name, ma_uint32 sampleRate, ma_uint32 channelCount,
	...
	mmw::Result decodeAudioFile(std::string filename, Sound& sound)
	{
		...
		else if (fileExtension == ".wav")
		{
			...
			uint32_t sampleRate{};
			uint64_t frameCount{};
			int16_t* samples = drwav_open_memory_and_read_pcm_frames_s16(
		...
		else if (fileExtension == ".flac")
		{
			...
			uint32_t sampleRate{};
			uint64_t frameCount{};
			int16_t* samples = drflac_open_memory_and_read_pcm_frames_s16(
			    bytes.data(), bytes.size(), &channels, &sampleRate, &frameCount, nullptr);
			if (samples == nullptr)
	...
	void SoundPool::initialize(const std::string& path, ma_engine* engine, ma_sound_group* group,
	                           SoundFlags flags)
	{
		constexpr ma_uint32 maSoundFlags = MA_SOUND_FLAG_NO_PITCH |
		                                   MA_SOUND_FLAG_NO_SPATIALIZATION | MA_SOUND_FLAG_DECODE |
		                                   MA_SOUND_FLAG_ASYNC;

		std::wstring wPath = IO::mbToWideStr(path);
		for (int i = 0; i < pool.size(); ++i)
		{
			ma_result result = ma_sound_init_from_file_w(engine, wPath.c_str(), maSoundFlags, group,
			                                             nullptr, &pool[i].source);
	...
	void SoundPool::play(float start, float end)
	{
		if (flags & SoundFlags::EXTENDABLE)
		{
			...
			const bool isNewSoundWithinOldRange =
			    mmw::isWithinRange(start, currentInstance.lastStartTime,
			                       currentInstance.lastEndTime) &&
