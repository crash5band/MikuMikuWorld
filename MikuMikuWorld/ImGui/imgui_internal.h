...
struct ImGuiDockNodeSettings;        // Storage for a dock node in .ini file (we preserve those even if the
                                     // associated dock node isn't active during the session)
...
struct ImGuiWindowTempData;          // Temporary storage for one window (that's the data which in theory we
                                     // could ditch at the end of the frame, in practice we currently keep it
                                     // for each window)
...
enum ImGuiLocKey : int;         // -> enum ImGuiLocKey              // Enum: a localization entry for
                                // translation.
typedef int ImGuiDataAuthority; // -> enum ImGuiDataAuthority_      // Enum: for storing the source
                                // authority (dock node vs window) of a field
...
#define IM_BITARRAY_CLEARBIT(_ARRAY, _N)                                                                               \
	((_ARRAY[(_N) >> 5] &= ~((ImU32)1 << ((_N) & 31)))) // Macro version of ImBitArrayClearBit(): ensure args have
...
enum ImGuiItemFlags_
{
	...
	ImGuiItemFlags_ButtonRepeat = 1 << 1,      // false     // Button() will return true multiple times
	...
	ImGuiItemFlags_MixedValue = 1 << 6, // false     // [BETA] Represent a mixed/indeterminate value, generally
...
enum ImGuiItemStatusFlags_
{
	...
	ImGuiItemStatusFlags_HoveredRect = 1 << 0, // Mouse position is within item rectangle (does NOT mean that the window
	                                           // is in correct z-order and can be hovered!, this is only one part of
	                                           // the most-common IsItemHovered test)
	...
	ImGuiItemStatusFlags_ToggledSelection = 1 << 3, // Set when Selectable(), TreeNode() reports toggling a selection.
	...
	ImGuiItemStatusFlags_Visible =
	    1 << 9, // [WIP] Set when item is overlapping the current clipping rectangle (Used internally.
...
enum ImGuiButtonFlagsPrivate_
{
	...
	ImGuiButtonFlags_PressedOnClickReleaseAnywhere = 1 << 6, // return true on click + release even if the release event
	...
	ImGuiButtonFlags_AlignTextBaseLine = 1 << 15, // vertically align button to match text baseline - ButtonEx() only //
	                                              // FIXME: Should be removed and handled by SmallButton(), not possible
	                                              // currently because of DC.CursorPosPrevLine
	...
	ImGuiButtonFlags_NoNavFocus = 1 << 18, // don't override navigation focus when activated (FIXME: this is essentially
	                                       // used everytime an item uses ImGuiItemFlags_NoNav, but because legacy specs
	                                       // don't requires LastItemData to be set ButtonBehavior(), we can't poll
	                                       // g.LastItemData.InFlags)
...
struct ImGuiNextItemData
{
	...
	ImGuiID FocusScopeId; // Set by SetNextItemMultiSelectData() (!= 0 signify value has been set, so
...
enum ImGuiInputFlags_
{
	...
	ImGuiInputFlags_LockUntilRelease = 1 << 7, // Access to key data will require EXPLICIT owner ID (ImGuiKeyOwner_Any/0
	                                           // will NOT accepted for polling). Cleared when the key is released or at
	                                           // end of each frame if key is released. This is useful to make
	                                           // input-owner-aware code steal keys from non-input-owner-aware code.
	...
	ImGuiInputFlags_RouteFocused = 1 << 8, // (Default) Register focused route: Accept inputs if window is in focus
	                                       // stack. Deep-most focused window takes inputs. ActiveId takes inputs over
...
enum ImGuiActivateFlags_
{
	...
	ImGuiActivateFlags_TryToPreserveState = 1 << 2, // Request widget to preserve state if it can (e.g. InputText will
...
enum ImGuiNavMoveFlags_
{
	...
	ImGuiNavMoveFlags_AllowCurrentNavId = 1
	                                      << 4, // Allow scoring and considering the current NavId as a move target
	                                            // candidate. This is used when the move source is offset (e.g. pressing
	                                            // PageDown actually needs to send a Up move request, if we are pressing
	...
	ImGuiNavMoveFlags_DontSetNavHighlight = 1
	                                        << 12, // Do not alter the visible state of keyboard vs mouse nav highlight
};
...
enum ImGuiDockNodeFlagsPrivate_
{
	...
	ImGuiDockNodeFlags_DockSpace =
	    1 << 10, // Local, Saved  // A dockspace is a node that occupy space within an existing user
	             // window. Otherwise the node is floating and create its own window.
	...
	ImGuiDockNodeFlags_NoTabBar = 1 << 12,     // Local, Saved  // Tab bar is completely unavailable. No
	                                           // triangle in the corner to enable it back.
	ImGuiDockNodeFlags_HiddenTabBar = 1 << 13, // Local, Saved  // Tab bar is hidden, with a triangle in the corner to
	                                           // show it again (NB: actual tab-bar instance may be destroyed as this is
	                                           // only used for single-window tab bar)
	ImGuiDockNodeFlags_NoWindowMenuButton = 1 << 14, // Local, Saved  // Disable window/docking menu (that one that
	                                                 // appears instead of the collapse button)
	ImGuiDockNodeFlags_NoCloseButton = 1 << 15, // Local, Saved  //
	ImGuiDockNodeFlags_NoDocking =
	    1 << 16, // Local, Saved  // Disable any form of docking in this dockspace or individual node.
	             // (On a whole dockspace, this pretty much defeat the purpose of using a dockspace at
	             // all). Note: when turned on, existing docked nodes will be preserved.
	ImGuiDockNodeFlags_NoDockingSplitMe = 1
	                                      << 17, // [EXPERIMENTAL] Prevent another window/node from splitting this node.
	...
	ImGuiDockNodeFlags_NoDockingOverOther = 1 << 20, // [EXPERIMENTAL] Prevent this node to be docked
	                                                 // over another window or non-empty node.
	ImGuiDockNodeFlags_NoDockingOverEmpty = 1 << 21, // [EXPERIMENTAL] Prevent this node to be docked over an empty node
	                                                 // (e.g. DockSpace with no other windows)
...
{
	ImGuiID ID;
	ImGuiDockNodeFlags SharedFlags;         // (Write) Flags shared by all nodes of a same dockspace
	...
	ImGuiDockNodeFlags MergedFlags;         // (Read)  Effective flags (== SharedFlags | LocalFlagsInNode |
	                                        // LocalFlagsInWindows)
	ImGuiDockNodeState State;
	ImGuiDockNode* ParentNode;
	ImGuiDockNode* ChildNodes[2];   // [Split node only] Child nodes (left/right or top/bottom).
	                                // Consider switching to an array.
	ImVector<ImGuiWindow*> Windows; // Note: unordered list! Iterate TabBar->Tabs for user-order.
	ImGuiTabBar* TabBar;
	ImVec2 Pos;                   // Current position
	ImVec2 Size;                  // Current size
	ImVec2 SizeRef;               // [Split node only] Last explicitly written-to size (overridden when using a
	...
	ImGuiWindow* VisibleWindow;         // Generally point to window which is ID is == SelectedTabID, but
	                                    // when CTRL+Tabbing this can be a different window.
	ImGuiDockNode* CentralNode;         // [Root node only] Pointer to central node.
	ImGuiDockNode* OnlyNodeWithWindows; // [Root node only] Set when there is a single visible node
	                                    // within the hierarchy.
	int CountNodeWithWindows;           // [Root node only]
	int LastFrameAlive;                 // Last frame number the node was updated or kept alive explicitly with
	                                    // DockSpace() + ImGuiDockNodeFlags_KeepAliveOnly
	int LastFrameActive;                // Last frame number the node was updated.
	int LastFrameFocused;               // Last frame number the node was focused.
	ImGuiID LastFocusedNodeId;          // [Root node only] Which of our child docking node (any ancestor in
	                                    // the hierarchy) was last focused.
	ImGuiID SelectedTabId;              // [Leaf node only] Which of our tab/window is selected.
	ImGuiID WantCloseTabId;             // [Leaf node only] Set when closing a specific tab/window.
	ImGuiDataAuthority AuthorityForPos : 3;
	ImGuiDataAuthority AuthorityForSize : 3;
	ImGuiDataAuthority AuthorityForViewport : 3;
	bool IsVisible : 1; // Set to false when the node is hidden (usually disabled as it has no
	                    // active window)
	bool IsFocused : 1;
	bool IsBgDrawnThisFrame : 1;
	bool HasCloseButton : 1; // Provide space for a close button (if any of the docked window has
	                         // one). Note that button may be hidden on window without one.
	bool HasWindowMenuButton : 1;
	bool HasCentralNodeChild : 1;
	bool WantCloseAll : 1; // Set when closing all tabs at once.
	bool WantLockSizeOnce : 1;
	bool WantMouseMove : 1; // After a node extraction we need to transition toward moving the newly
	                        // created host window
...
// A better solution may involve appending into a log of colors in ImGuiContext + store offsets into
// those arrays in ImGuiWindow, but it would be more complex as we'd need to double-buffer both as
// e.g. drop target may refer to window from last frame.
...
struct ImGuiViewportP : public ImGuiViewport
{
	...
	int LastFocusedStampCount; // Last stamp number from when a window hosted by this viewport was
	...
	float LastAlpha;
	bool LastFocusedHadNavWindow; // Instead of maintaining a LastFocusedWindow (which may harder to
	                              // correctly maintain), we merely store weither NavWindow != NULL
	...
	ImGuiViewportP()
	{
		Window = NULL;
		Idx = -1;
		LastFrameActive = DrawListsLastFrame[0] = DrawListsLastFrame[1] = LastFocusedStampCount = -1;
		LastNameHash = 0;
...
{
	ImGuiID ID;
	ImVec2ih Pos; // NB: Settings position are stored RELATIVE to the viewport! Whereas runtime ones
	              // are absolute positions.
	ImVec2ih Size;
	ImVec2ih ViewportPos;
	ImGuiID ViewportId;
	ImGuiID DockId;  // ID of last known DockNode (even if the DockNode is invisible because it has
	                 // only 1 active window), or 0 if none.
	ImGuiID ClassId; // ID of window class if specified
	short DockOrder; // Order of the last time the window was visible within its DockNode. This is
	                 // used to reorder windows that are reappearing on the same frame. Same value
	                 // between windows that were active and windows that were none are possible.
	bool Collapsed;
...
enum ImGuiDebugLogFlags_
{
	...
	ImGuiDebugLogFlags_EventMask_ =
	    ...
	    ImGuiDebugLogFlags_EventNav | ImGuiDebugLogFlags_EventClipper | ImGuiDebugLogFlags_EventSelection |
	    ImGuiDebugLogFlags_EventIO | ImGuiDebugLogFlags_EventDocking | ImGuiDebugLogFlags_EventViewport,
...
struct ImGuiStackLevelInfo
{
	...
	char Desc[57]; // Arbitrarily sized buffer to hold a result (FIXME: could replace Results[] with
	               // a chunk stream?) FIXME: Now that we added CTRL+C this should be fixed.
...
struct ImGuiContext
{
	...
	ImGuiWindow* MovingWindow;   // Track the window we clicked on (in order to preserve focus). The actual
	                             // window that is moved is generally MovingWindow->RootWindowDockTree.
	ImGuiWindow* WheelingWindow; // Track the window we started mouse-wheeling on. Until a timer elapse or
	...
	ImGuiID DebugHookIdInfo; // Will call core hooks: DebugHookIdInfo() from GetID functions, used by
	...
	ImGuiViewportP* CurrentViewport;     // We track changes of viewport (happening in Begin) so we can
	                                     // call Platform_OnChangedViewport()
	ImGuiViewportP* MouseViewport;
	ImGuiViewportP* MouseLastHoveredViewport; // Last known viewport that was hovered by mouse (even
	                                          // if we are not hovering any viewport any more) +
	                                          // honoring the _NoInputs flag.
	ImGuiID PlatformLastFocusedViewportId;
	ImGuiPlatformMonitor FallbackMonitor; // Virtual monitor used as fallback if backend doesn't
	                                      // provide monitor information.
	int ViewportFocusedStampCount;        // Every time the front-most window changes, we stamp its
	                                      // viewport with an incrementing counter
	...
	bool NavMousePosDirty;           // When set we will update mouse position if (io.ConfigFlags &
	                                 // ImGuiConfigFlags_NavEnableSetMousePos) if set (NB: this not enabled by
	                                 // default)
	...
	ImGuiNavItemData NavMoveResultLocalVisible; // Best move request candidate within NavWindow that
	                                            // are mostly visible (when using
	                                            // ImGuiNavMoveFlags_AlsoScoreVisibleSet flag)
	ImGuiNavItemData NavMoveResultOther;        // Best move request candidate within NavWindow's flattened hierarchy
	...
	ImGuiWindow* NavWindowingTarget;         // Target window when doing CTRL+Tab (or Pad Menu + FocusPrev/Next),
...
struct IMGUI_API ImGuiWindow
{
	...
	ImVec2 ViewportPos;                     // We backup the viewport position (since the viewport may disappear or
	...
	ImGuiWindow* RootWindowDockTree;             // Point to ourself or first ancestor that is not a child
	                                             // window. Cross through dock nodes.
	ImGuiWindow* RootWindowForTitleBarHighlight; // Point to ourself or first ancestor which will display
	...
	ImGuiWindow* NavLastChildNavWindow;      // When going to the menu bar, we remember the child window we came
	...
	ImVec2 NavPreferredScoringPosRel[ImGuiNavLayer_COUNT]; // Preferred X/Y position updated when
	                                                       // moving on a given axis, reset to FLT_MAX.
	...
	// Docking
	bool DockIsActive : 1; // When docking artifacts are actually visible. When this is set,
	                       // DockNode is guaranteed to be != NULL. ~~ (DockNode != NULL) &&
	                       // (DockNode->Windows.Size > 1).
	bool DockNodeIsVisible : 1;
	bool DockTabIsVisible : 1; // Is our window visible this frame? ~~ is the corresponding tab
	                           // selected?
	...
	ImGuiDockNode* DockNode;       // Which node are we docked into. Important: Prefer testing DockIsActive in many
	                               // cases as this will still be set when the dock node is hidden.
	ImGuiDockNode* DockNodeAsHost; // Which node are we owning (for parent windows)
	ImGuiID DockId;                // Backup of last valid DockNode->ID, so single window remember their dock node
	                               // id even when they are not bound any more
...
enum ImGuiTabItemFlagsPrivate_
{
	...
	ImGuiTabItemFlags_Unsorted = 1 << 22, // [Docking] Trailing tabs with the _Unsorted flag will be
...
struct ImGuiTableInstanceData
{
	...
	float LastFirstRowHeight; // Height of first row from last frame (FIXME: this is used as "header
	                          // height" and may be reworked)
...
struct IMGUI_API ImGuiTable
{
	...
	ImBitArrayPtr EnabledMaskByIndex;        // Column Index -> IsEnabled map (== not hidden by user/api) in a format
	...
	ImDrawListSplitter* DrawSplitter; // Shortcut to TempData->DrawSplitter while in table. Isolate draw commands
...
struct ImGuiTableSettings
{
	...
	ImGuiTableColumnIdx ColumnsCountMax; // Maximum number of columns this settings instance can store, we can
...
namespace ImGui
{
	...
	IMGUI_API void Shutdown(); // Since 1.60 this is a _private_ function. You can call DestroyContext() to destroy
	...
	IMGUI_API void MarkItemEdited(ImGuiID id); // Mark data associated to given item as "edited",
	...
	IMGUI_API void LogBegin(ImGuiLogType type,
	                        int auto_open_depth);         // -> BeginCapture() when we design v2 api, for
	...
	IMGUI_API void ActivateItem(ImGuiID id); // Remotely activate a button, checkbox, tree node etc. given its
	                                         // unique ID. activation is queued and processed on the next frame
	                                         // when the item is encountered again.
	...
	// - Ownership is most often claimed as a result of reacting to a press/down event (but
	// occasionally may be claimed ahead).
	...
	// - SetItemKeyOwner() is a shortcut for common simple case. A custom widget will probably want
	// to call SetKeyOwner() multiple times directly based on its interaction state.
	// - This is marked experimental because not all widgets are fully honoring the Set/Test idioms.
	// We will need to move forward step by step.
	...
	// - Expected to be later promoted to public API, the prototypes are designed to replace
	// existing ones (since owner_id can default to Any == 0)
	...
	//   Legacy functions use ImGuiKeyOwner_Any meaning that they typically ignore ownership, unless
	//   a call to SetKeyOwner() explicitly used ImGuiInputFlags_LockThisFrame or
	//   ImGuiInputFlags_LockUntilRelease.
	// - Binding generators may want to ignore those for now, or suffix them with Ex() until we
	// decide if this gets moved into public API.
	...
	//   values.
	// - When using one of the routing flags (e.g. ImGuiInputFlags_RouteFocused): routes requested
	// ahead of time given a chord (key + modifiers) and a routing policy.
	...
	// - Route is granted to a single owner. When multiple requests are made we have policies to
	// select the winning route.
	// - Multiple read sites may use the same owner id and will all get the granted route.
	// - For routing: when owner_id is 0 we use the current Focus Scope ID as a default owner in
	...
	IMGUI_API void DockContextProcessUndockWindow(ImGuiContext* ctx, ImGuiWindow* window,
	                                              bool clear_persistent_docking_ref = true);
	IMGUI_API void DockContextProcessUndockNode(ImGuiContext* ctx, ImGuiDockNode* node);
	IMGUI_API bool DockContextCalcDropPosForDocking(ImGuiWindow* target, ImGuiDockNode* target_node,
	...
	// - The DockBuilderXXX functions are designed to _eventually_ become a public API, but it is
	// too early to expose it and guarantee stability.
	// - Do not hold on ImGuiDockNode* pointers! They may be invalidated by any split/merge/remove
	// operation and every frame.
	// - To create a DockSpace() node, make sure to set the ImGuiDockNodeFlags_DockSpace flag when
	// calling DockBuilderAddNode().
	//   You can create dockspace nodes (attached to a window) _or_ floating nodes (carry its own
	//   window) with this API.
	// - DockBuilderSplitNode() create 2 child nodes within 1 node. The initial node becomes a
	// parent node.
	// - If you intend to split the node immediately after creation using DockBuilderSplitNode(),
	// make sure
	//   to call DockBuilderSetNodeSize() beforehand. If you don't, the resulting split sizes may
	//   not be reliable.
	...
	IMGUI_API void
	DockBuilderRemoveNodeChildNodes(ImGuiID node_id); // Remove all split/hierarchy. All remaining docked windows will
	                                                  // be re-docked to the remaining root node (node_id).
	IMGUI_API void DockBuilderSetNodePos(ImGuiID node_id, ImVec2 pos);
	IMGUI_API void DockBuilderSetNodeSize(ImGuiID node_id, ImVec2 size);
	IMGUI_API ImGuiID
	DockBuilderSplitNode(ImGuiID node_id, ImGuiDir split_dir, float size_ratio_for_node_at_dir, ImGuiID* out_id_at_dir,
	...
	// - Selection patterns generally need to react (e.g. clear a selection) when landing on one
	// item of the set.
	//   So in order to identify a set multiple lists in same window may each need a focus scope.
	//   If you imagine an hypothetical BeginSelectionGroup()/EndSelectionGroup() api, it would
	//   likely call PushFocusScope()/EndFocusScope()
	...
	IMGUI_API int TableGetHoveredColumn(); // May use (TableGetColumnFlags() & ImGuiTableColumnFlags_IsHovered)
	...
	// Render helpers
	// AVOID USING OUTSIDE OF IMGUI.CPP! NOT FOR PUBLIC CONSUMPTION. THOSE FUNCTIONS ARE A MESS.
	...
	// To use them externally (for custom widget) you may need an "extern template" statement in
	// your code in order to link to existing instances and silence Clang warnings (see #2036). e.g.
	// " extern template IMGUI_API float RoundScalarWithFormatT<float, float>(const char* format,
	// ImGuiDataType data_type, float v); "
	...
	IMGUI_API void DebugLocateItemOnHover(ImGuiID target_id); // Only call on reaction to a mouse Hover: because
	...
	//  (Old) IMGUI_VERSION_NUM  < 18209: using 'ItemAdd(....)'                              and
	//  'bool tab_focused = FocusableItemRegister(...)' (Old) IMGUI_VERSION_NUM >= 18209: using
	//  'ItemAdd(..., ImGuiItemAddFlags_Focusable)'  and 'bool tab_focused = (GetItemStatusFlags() &
	...
	// Widget code are simplified as there's no need to call FocusableItemUnregister() while
	// managing the transition from regular widget to TempInputText()
